...
const Cart = () => {
    ...
    useEffect(() => {
        async function makeRequestToServer() {
            try {
                const response = await axios.post("https://martini-e-commerce-api.onrender.com/api/checkout/payment", { tokenId: stripeClientToken.id, amount: cart.cartTotalAmount * 100 });
